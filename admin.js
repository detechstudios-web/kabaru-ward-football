...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadResultFixtures() {
        ...
        try {
            ...
            availableFixtures.forEach(function (fixture) {
    ...
    function updateGoalWarnings() {
        ...
        const homeTotal =
            Number(homeScore.value) || 0;

        const awayTotal =
            Number(awayScore.value) || 0;
                ...
                try {
                    ...
                    if (allGoals.length > 0) {
                        ...
                        if (goalError) {
                            ...
                            // Try to remove the result
                            // if goal insertion fails.
                            await supabaseClient
                                .from("results")
                                .delete()
                                .eq(
...
if (competitionForm) {
    ...
    competitionForm.addEventListener("submit", async function (event) {

        event.preventDefault();

        const message = document.getElementById("competitionFormMessage");
        ...
        const season =
            document.getElementById("competitionSeason").value.trim();

        const startDate =
            document.getElementById("competitionStartDate").value || null;

        const endDate =
            document.getElementById("competitionEndDate").value || null;

        const status =
            document.getElementById("competitionStatus").value;

        const description =
            document.getElementById("competitionDescription").value.trim();

        message.textContent = "Saving competition...";
